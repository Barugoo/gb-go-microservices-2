...
type flusherWriter struct{ *responseWriter }
type hijackerWriter struct{ *responseWriter }
type readerFromWriter struct{ *responseWriter }
type pusherWriter struct{ *responseWriter }
...
func init() {
	...
	builders[hijacker+pusher] = func(w *responseWriter) measurable {
		return struct {
			*responseWriter
			http.Pusher
			http.Hijacker
		}{w, pusherWriter{w}, hijackerWriter{w}}
	}
	builders[hijacker+readerFrom] = func(w *responseWriter) measurable {
		return struct {
			*responseWriter
			io.ReaderFrom
			http.Hijacker
		}{w, readerFromWriter{w}, hijackerWriter{w}}
	}
	...
	builders[hijacker+readerFrom+pusher] = func(w *responseWriter) measurable {
		return struct {
			*responseWriter
			http.Pusher
			io.ReaderFrom
	...
	builders[flusher+hijacker+readerFrom+pusher] = func(w *responseWriter) measurable {
		return struct {
			*responseWriter
			http.Pusher
			io.ReaderFrom
...
var (
	MeasurableHandler = func(h http.HandlerFunc) http.HandlerFunc {
